...
use std::path::Path;
use crate::content::{ContentLoader, ChapterData, ExerciseData, StepData};
...
impl VimTutorialGame {
    pub fn new() -> Result<Self> {
        ...
        if let Ok(mut file) = OpenOptions::new()
            ...
            let _ = writeln!(file, "Started at: {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S"));
        }
    ...
    fn show_chapter_menu(&mut self) -> Result<()> {
        loop {
            ...
            println!("章番号を選択してください (1-{}, q=終了):", self.content_loader.get_chapter_count());
            print!("選択: ");
            io::stdout().flush()?;
            ...
            match io::stdin().read_line(&mut input) {
                ...
                Ok(_) => {
                    ...
                    if let Ok(chapter_num) = input.parse::<u8>() {
                        if chapter_num >= 1 && chapter_num <= self.content_loader.get_chapter_count() as u8 {
                            return self.start_chapter(chapter_num);
                        } else {
                            println!("❌ 無効な章番号です。1-{} の範囲で入力してください。", self.content_loader.get_chapter_count());
                        }
                    } else {
    ...
    fn start_chapter(&mut self, chapter_num: u8) -> Result<()> {
        if let Some(chapter) = self.content_loader.get_chapter(chapter_num) {
            ...
            println!("\n🎯 === 第{}章: {} ===", chapter.chapter.number, chapter.chapter.title);
...
"#, status_file, step.expected_input);
        ...
        let kill_result = Command::new("tmux")
            ...
            .output()?;
        debug_log!("セッション削除結果: success={}, stderr={}", 
        ...
        let new_session_result = Command::new("tmux")
            ...
            .output()?;
        debug_log!("セッション作成結果: success={}, stderr={}", 
        ...
        if !new_session_result.status.success() {
            ...
            return Err(anyhow::anyhow!("tmuxセッション作成に失敗: {}", 
                String::from_utf8_lossy(&new_session_result.stderr)));
        }
        ...
        if !split_result.status.success() {
            ...
            return Err(anyhow::anyhow!("tmux画面分割に失敗: {}", 
                String::from_utf8_lossy(&split_result.stderr)));
        }
        ...
        debug_log!("解析されたペイン: 上={:?}, 下={:?}", panes.get(0), panes.get(1));
        
        let top_pane = panes.get(0).ok_or_else(|| anyhow::anyhow!("上ペインが見つかりません"))?;
        let bottom_pane = panes.get(1).ok_or_else(|| anyhow::anyhow!("下ペインが見つかりません"))?;
        ...
        let nvim_result = Command::new("tmux")
            ...
            .output()?;
        debug_log!("Neovim起動結果: success={}, stderr={}", 
        ...
        if !nvim_result.status.success() {
            ...
            return Err(anyhow::anyhow!("Neovim起動に失敗: {}", 
                String::from_utf8_lossy(&nvim_result.stderr)));
        }
        ...
        let after_nvim = Command::new("tmux")
            ...
            .output()?;
        debug_log!("Neovim起動後ペイン: {}", String::from_utf8_lossy(&after_nvim.stdout));
        ...
        let after_focus = Command::new("tmux")
            .args(["list-panes", "-t", session_name])
            .output()?;
        debug_log!("フォーカス後ペイン: {}", String::from_utf8_lossy(&after_focus.stdout));
        ...
        let before_layout = Command::new("tmux")
            .args(["list-panes", "-t", session_name])
            .output()?;
        debug_log!("レイアウト前ペイン: {}", String::from_utf8_lossy(&before_layout.stdout));
        ...
        let final_panes = Command::new("tmux")
            .args(["list-panes", "-t", session_name, "-F", "#{pane_index}:#{pane_id}:#{pane_active}:#{pane_height}"])
            .output()?;
        debug_log!("最終ペイン状態: {}", String::from_utf8_lossy(&final_panes.stdout));
        
        // ウィンドウの状態も確認
        let window_info = Command::new("tmux")
            .args(["list-windows", "-t", session_name, "-F", "#{window_index}:#{window_layout}"])
            .output()?;
        debug_log!("ウィンドウ状態: {}", String::from_utf8_lossy(&window_info.stdout));
                            ...
                            if let (Ok(line_num), Ok(col_num)) = (
                                ...
                                if current_position != last_position {
                                    ...
                                    if current_position == target_position && !success_triggered {
                                        debug_log!("🎉 目標達成！カーソル位置: {}行{}列", line_num, col_num);
                                        ...
                                        debug_log!("カーソル位置: {}行{}列 (目標: {}行{}列)", 
                                               line_num, col_num, target_position.0, target_position.1);
                                    }
